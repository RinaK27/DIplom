...
class SelectionByClosedApplications {
    ...
    @Test
    fun selectionByClosedApplications() {
        Thread.sleep(5000)
        ...
        val materialCheckBox3 = onView(
            allOf(
                withId(R.id.item_filter_cancelled), withText("Cancelled"),
                childAtPosition(
                    childAtPosition(
                        withClassName(`is`("android.widget.ScrollView")),
                        0
        ...
        Thread.sleep(2000)
        val textView = onView(
            allOf(withId(R.id.status_label_text_view), withText("Canceled"), isDisplayed())
        )
        textView.check(matches(withText("Canceled")))
    }
