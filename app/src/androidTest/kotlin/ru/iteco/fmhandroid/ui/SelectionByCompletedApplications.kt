...
class SelectionByCompletedApplications {
    ...
    @Test
    fun selectionByCompletedApplications() {
        Thread.sleep(5000)
        ...
        val materialCheckBox3 = onView(
            allOf(
                withId(R.id.item_filter_executed), withText("Executed"),
                childAtPosition(
                    childAtPosition(
                        withClassName(`is`("android.widget.ScrollView")),
                        0
        ...
        Thread.sleep(2000)
        val textView = onView(
            allOf(withId(R.id.status_label_text_view), withText("Executed"), isDisplayed())
        )
        textView.check(matches(withText("Executed")))
    }
